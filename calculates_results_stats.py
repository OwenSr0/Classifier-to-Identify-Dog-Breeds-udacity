...
#                                                                             
# PROGRAMMER:
# DATE CREATED: Owen Sanchez                           
# REVISED DATE: Saturday, 8 of july 2023
...
def calculates_results_stats(results_dic):
    ...
    n_dogs_img = sum(element[3] for element in results_dic.values())
    n_notdogs_img = n_images - n_dogs_img

    n_match = sum(element[2] for element in results_dic.values())

    n_correct_dogs = sum(element[4] for element in results_dic.values() if element[3])
    n_correct_notdogs = sum(1 for element in results_dic.values() if not element[3] and not element[4])
    n_correct_breed = sum(element[2] and element[3] for element in results_dic.values())
